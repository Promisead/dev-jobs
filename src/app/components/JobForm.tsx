...
export default function JobForm({
  ...
  /*
   * The description now contains
   * rich HTML instead of plain text.
   */
  ...
  const [isDescriptionImageUploading, setIsDescriptionImageUploading] =
    useState(false);

  const isImageUploading =
    isJobIconUploading ||
    isContactPhotoUploading ||
    isDescriptionImageUploading;
  ...
  async function handleSaveJob(data: FormData) {
    ...
    if (isImageUploading) {
      setErrors((current) => ({
        ...
        form: "Please wait for all image uploads to finish before publishing the job.",
      }));
  ...
  return (
    <Theme>
      <form
        ...
        className="container mt-6 flex flex-col gap-6"
      >
        {jobDoc && <input type="hidden" name="id" value={jobDoc._id} />}

        {/*
          The actual form value submitted
          for description.
        */}
        ...
        <div>
          <div className="mb-2">
            ...
            <p className="mt-1 text-xs text-gray-500">
              Format the listing exactly as applicants should see it.
            </p>
          </div>
          ...
          <RichTextEditor
            ...
            onUploadingChange={setIsDescriptionImageUploading}
          />
        ...
        <div className="flex justify-center pt-4">
          <button
            ...
            disabled={isSaving || isImageUploading}
            aria-busy={isSaving || isImageUploading}
            className="inline-flex min-w-[200px] items-center justify-center gap-2 rounded-lg bg-[#077998] px-8 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#066982] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {(isSaving || isImageUploading) && <InlineLoader />}
